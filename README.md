...
data\
  ...
  xray.exe            TLS-fragment proxy; also runs the TUN tunnel (Xray TUN)
  wintun.dll           Wintun driver used by xray TUN mode
...
- Runs an **Xray-core TUN instance** (`xray.exe` + `wintun.dll`) with a
  `TorJetTun` adapter and `autoRoute` — all system traffic enters the tunnel.
- Routing rules: `tor.exe` and `xray.exe` bypass the tunnel by process (the
  tor core talks to relays/bridges directly, no route games); **DNS (port 53,
  TCP+UDP) is hijacked** into Xray's DNS module and resolved through tor's
  own DNSPort — no DNS leak by construction; **UDP is dropped** (tor has no
  UDP); everything else goes through tor's SOCKS port.
- IPv6 is blocked system-wide while tunnelled (kill switch) and restored on
  teardown; DNS caches are flushed on enable and teardown.
- TUN turns off automatically if tor stops, the helper exits, or the tunnel
  process dies; routes and IPv6 are restored.
...
all apps
   ...
   ▼
xray.exe (TUN: DNS hijacked, UDP blocked, tor.exe excluded by process)
...
wintun) keep their own licenses.
